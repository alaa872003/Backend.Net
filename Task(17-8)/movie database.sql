...
constraint fk_dir foreign key (dir_id) 
 ...
 constraint fk_mov foreign key (mov_id) references movieSchema.movie(mov_id) on delete cascade
)
...
constraint fk_act foreign key (act_id) references movieSchema.actor(act_id) on delete cascade,

 constraint fk_mov_cast foreign key (mov_id) references movieSchema.movie(mov_id) on delete cascade
)
...
constraint fk_gen foreign key (gen_id) references movieSchema.genres(gen_id) on delete cascade
)
...
constraint fk_rev foreign key (rev_id) references movieSchema.reviewer(rev_id) on delete cascade
)